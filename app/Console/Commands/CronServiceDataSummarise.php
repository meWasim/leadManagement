...
class CronServiceDataSummarise extends Command
{
  /**
   ...
   * @var string
   */
  protected $signature = 'CronServiceDataSummarise {--date=}';
   ...
   * @var string
   */
  protected $description = 'The Value calculate from Summery_history table and sum of mt success Then Save on Table Thats Means Organize Data';
  ...
  */
  public function handle()
  {
    ...
    $dateforData = new Carbon($date);
    $year = $dateforData->format('Y');
    $month = $dateforData->format('m');
    ...
    $this->call('CronPnlSummariseOperator',[
      '--date' => $date_input
    ] );

    $this->call('CronPnlUserSummariseOperator',[
      '--date' => $date_input
    ] );

    $this->call('CronDashboardSummarizeDay');

    $this->call('CronMonthlyReportSummery',[
