...
class CronServiceDataDaySync extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'CronServiceSyncDayData {--date=}';
     ...
     * @var string
     */
    protected $description = 'The Cron Will be Run for a Daily Date Wise update Data Like mt_sucess,mt_failed, reg';
    ...
    public function handle()
    {
        ...
        $description = 'The Cron Will be Run for a Daily Date Wise date='.$start_date.' update Data Like mt_sucess,mt_failed, reg';
        $data = ['description' => $description,'signature' => $this->signature,'command' => 'php artisan '.$this->signature,'date' => $start_date,'cron_start_date' => $cron_start_date->format('Y-m-d H:i:s'),'cron_end_date' => $cron_start_date->format('Y-m-d H:i:s'),'total_in_up' => $loop,'table_name' => 'service_histories','status' => 'processing'];

        CronLog::upsert($data,['id'],['description','signature','command','date','cron_start_date','cron_end_date','table_name','total_in_up','status']);
        
        //$serviceStaticIdsTest =  array(2121,2122,2123,2124,1820,1821);
        ...
        $data = ['description' => $description.$LastServiceID,'signature' => $this->signature,'command' => 'php artisan '.$this->signature,'date' => $start_date,'cron_start_date' => $cron_start_date->format('Y-m-d H:i:s'),'cron_end_date' => $cron_end_date->format('Y-m-d H:i:s'),'total_in_up' => $totalRowInsert,'table_name' => 'service_histories','status' => $status];
        ...
        $this->call('CronServiceDataSummarise',[
            '--date' => $date_input
        ] );

        $this->call('CronUserServiceDataSummarise',[
            '--date' => $date_input
        ] );
