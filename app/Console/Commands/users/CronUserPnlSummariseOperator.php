...
class CronUserPnlSummariseOperator extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'CronPnlUserSummariseOperator {--date=}';
     ...
     * @var string
     */
    protected $description = 'User PNL Summery Summarise';
    ...
    public function handle()
    {
        ...
        $data = ['description' => $this->description,'signature' => $this->signature,'command' => 'php artisan '.$this->signature,'date' => $start_date->format('Y-m-d'),'cron_start_date' => $start_date->format('Y-m-d H:i:s'),'cron_end_date' => $start_date->format('Y-m-d H:i:s'),'total_in_up' => 0,'table_name' => 'reports_pnls_operator_summarizes','status' => 'Processing'];
        ...
        foreach($UserOperatorServices as $Uasrkey => $UserOperatorService){
            ...
            if(!empty($RecordsByDate))
            {
                ...
                foreach ($RecordsByDate as $key => $value)
                {
                    ...
                    {
                        //$TmpmatchedReports['opearator_id'] = $opearator_id;
                        //$TmpmatchedReports[$opearator_id]=$value;
