...
class CronUserServiceDataSummarise extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'CronUserServiceDataSummarise {--date=} {--uid=}';
     ...
     * @var string
     */
    protected $description = 'The Value calculate from Service_history  table each User and sum of mt success Then Save in  Table report_summerise_users - According to Select Operators & Services for user id ';
    ...
    public function handle()
    {
        ...
        $uid = $this->option('uid');
        $date = $date_input;// "2022-12-13";//$date_input;
        ...
        $status = 'Processing';
        $description = 'The Value calculate from Service_history  table each User and sum of mt success Then Save in  Table report_summerise_users - According to Select Operators & Services for user id ';
        $data = ['description' => $description,'signature' => $this->signature,'command' => 'php artisan '.$this->signature,'date' => $date,'cron_start_date' => $cron_start_date->format('Y-m-d H:i:s'),'cron_end_date' => $cron_start_date->format('Y-m-d H:i:s'),'total_in_up' => 0,'table_name' => 'report_summerise_users','status' => $status];
        ...
        foreach($operators as $Uasrkey=>$operator){
            ...
            if(!empty($Operators))
            {
                ...
                // dd($user->toArray());
                $records += sizeof($serviceSummarise);
            }

            $description = $description.$Uasrkey.',';
        }

        print_r($records." Records Insert/Updated \n");
        $cron_end_date = new DateTime('now');
        $data = ['description' => $description,'signature' => $this->signature,'command' => 'php artisan '.$this->signature,'date' => $date,'cron_start_date' => $cron_start_date->format('Y-m-d H:i:s'),'cron_end_date' => $cron_end_date->format('Y-m-d H:i:s'),'total_in_up' => $records ,'table_name' => 'report_summerise_users','status' => $status];
