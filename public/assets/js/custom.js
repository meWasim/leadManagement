...
$(document).ready(function() {
    ...
    $('.date').on('apply.daterangepicker', function(ev, picker) {
        ...
        $('#date'+p_id).val(picker.startDate.format('YYYY-MM-DD'));
    });
...
function commonLoader() {
    ...
    $('.status_btn').on('change', function() {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'JSON',
            url: baseUrl+'management/update_operator',
            data:{
    ...
    function RevenueCal() {
        var operator_share =document.getElementById("opt-share").value; //$(".opt-share").val();
        var merchent_share = 100-operator_share;

        document.querySelector('input[name="merchant_revenue_share"]').value = merchent_share;
        // document.getElementById("mrch_share").append(merchent_share);
    }
    function confirmPassword(){
        var e = document.getElementById("password");
        var password = e.value;
        var e = document.getElementById("confirm_password");
        var confirm_password = e.value;
    ...
    function RevenurSubmit(){
        var optshare = $("#opt-share").val();
    if (optshare == '') {
        $("#erroropt-share").removeClass("gu-hide");
        return false;
    } else {
        $("#erroropt-share").addClass("gu-hide");
    }
    var year = $("#year").val();
    if (year == '') {
        $("#erroryear").removeClass("gu-hide");
        return false;
    } else {
        $("#erroryear").addClass("gu-hide");
    }
    var month = $("#month").val();
    if (month == '') {
        $("#errormonth").removeClass("gu-hide");
        return false;
    } else {
        $("#errormonth").addClass("gu-hide");
    }
    }
    function reset()
    {
