...
class CronServiceDataRangeSummariseManual extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'CronServiceDataRangeSummariseManual';
     ...
     * @var string
     */
    protected $description = 'The Value calculate from Summery_history table by date range and sum of mt success Then Save on Table Thats Means Organize Data - Inpute date range from keyboard';
    ...
    public function handle()
    {
        ...
        foreach ($period as $date) {
            ...
            $records = 0;
            $description = 'The Value calculate from Summery_history table by date '.$date.'and sum of mt success Then Save on Table Thats Means Organize Data - Inpute date range from keyboard';
            $status = 'Failure';
            ...
            if(!empty($Operators))
            {
                ...
                if(sizeof($serviceSummarise)>0)
                {
                    ...
                    report_summarize::upsert($serviceSummarise,['operator_id','date'],['operator_id','operator_name','country_id','date','fmt_success','fmt_failed','mt_success','mt_failed','gros_rev','total_reg','total_unreg','total','purge_total','currency']);

                    $status = 'success';
                }

                print_r($records. " Records Insert/Updated date= ".$date."\n");
