...
class CronPnlSummariseOperatorRangeSync extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'CronPnlSummariseOperatorRangeSync {--sdate=} {--edate=}';
    ...
    public function handle()
    {
        ...
        $data = ['description' => $descriptionCron,'signature' => $this->signature,'command' => 'php artisan '.$this->signature,'date' => $rangeStartDate,'cron_start_date' => $start_date->format('Y-m-d H:i:s'),'cron_end_date' => $start_date->format('Y-m-d H:i:s'),'total_in_up' => 0,'table_name' => 'reports_pnls_operator_summarizes','status' => 'Processing'];
        ...
        foreach ($period as $date) {
            ...
            $total_dates = $total_dates.$date_input."_";
            Utility::CronLog("Data inserting date : ".$date_input);
        ...
        $data = ['description'=>$descriptionCron,'signature'=>$this->signature,'command'=>'php artisan '.$this->signature,'date'=>$rangeStartDate,'cron_start_date'=>$start_date->format('Y-m-d H:i:s'),'cron_end_date'=>$end_date->format('Y-m-d H:i:s'),'total_in_up'=>$totelDataInsert,'table_name'=>'reports_pnls_operator_summarizes','status'=> $status];
        CronLog::upsert($data,['signature','date'],['description','command','cron_start_date','cron_end_date','table_name','total_in_up','status']);
